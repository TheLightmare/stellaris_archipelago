...
// IMPORTANT: every proxied export MUST call trigger_deferred_init().
// We can't predict which version.dll function Stellaris will call first
// (varies by Windows version, game build, and locale). Modern apps use
// the W variants; if only the A variants triggered init, the bridge
// thread (and therefore the named pipe) would never start on those
// systems — exactly the silent-DLL bug seen in the wild.
// trigger_deferred_init() uses InterlockedCompareExchange and is safe
// to call from any thread, any number of times — first call wins.
